...
namespace GcSite.ModuleSys.Controllers
{
    public class HomeController : Controller
    {
        ...
        public ActionResult PostReportData(string date)
        {
            ...
            return Json(new { sourceSite, keyword, territory, networkDevice }, JsonRequestBehavior.AllowGet);
        }
        //[Route("~/Home/")]
        ...
        public List<FlowComputer> PageviewByDay(DateTime endTime, string date, int site)
        {
            ...
            try
            {
                ...
                if (date == "0" || date == "-1")
                {
                    date = date == "0" ? "0" : date;
                    endTime = DateTime.Now.AddDays(double.Parse(date));
                    vistio = work.CreateRepository<FlowComputer>().GetList(m => DbFunctions.DiffDays(
                        m.CurrentTime, endTime) == 0 && m.WebInfo.Id == site
                        ).ToList();
                }
                else
                {
